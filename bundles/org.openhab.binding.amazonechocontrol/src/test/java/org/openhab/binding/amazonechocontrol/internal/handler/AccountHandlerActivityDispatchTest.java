 ...
 * echo handlers: the startup guard on the push path, the history replay of an explicit refresh, and the state the
 * refresh switch shows while a request runs.
 *
...
public class AccountHandlerActivityDispatchTest {
    ...
    public void testRefreshSwitchShowsOnWhileTheRequestRunsAndOffAfterwards() {
        ...
        InOrder order = inOrder(accountCallback);
    ...
    public void testActivityRequestUsesTheConfiguredWindow() {
        ...
        long defaultWindowMillis = 120_000;
        long futureCushionMillis = 30_000;
        assertTrue(start.getValue() >= before - defaultWindowMillis && start.getValue() <= after - defaultWindowMillis,
